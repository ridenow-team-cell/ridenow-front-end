...
import { ChevronDown, ChevronUp, Clock, Trash2, Plus } from 'lucide-react';
...
const AddScheduleModal: React.FC<AddScheduleModalProps> = ({
    ...
    const [formData, setFormData] = useState<ScheduleFormData>({
        route: initialData?.route || 'Route 1',
        bus: initialData?.bus || 'Bus 01',
        driver: initialData?.driver || 'John Doe',
        departureTime: initialData?.departureTime || '08:00 AM',
        arrivalTime: initialData?.arrivalTime || '04:30 PM',
        selectedDays: initialData?.selectedDays || ['Mon', 'Tue', 'Wed', 'Thu', 'Fri'],
        numberOfSeats: initialData?.numberOfSeats || 24,
        seatLayout: initialData?.seatLayout || initializeSeatLayout(),
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <div className="relative">
                            ...
                            <Clock size={20} className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-400" />
                        </div>
                    </div>
                ...
                <div className="flex w-full  gap-4 pt-4">
                    <button
                        type="button"
                        onClick={onClose}
                        className="px-8 py-3 bg-gray-200 text-gray-800 rounded-lg text-base font-medium hover:bg-gray-300 transition-colors w-full"
                    >
                        Cancel
                    </button>
                    <button
                        type="submit"
                        className="px-8 py-3 bg-[#E7A533] text-gray-800 rounded-lg text-base font-medium hover:bg-[#d69420] transition-colors w-full"
                    >
                        Save Schedule
                    </button>
                </div>
