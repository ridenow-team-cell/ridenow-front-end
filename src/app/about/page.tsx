...
const ThemeToggle: React.FC<{ isDark: boolean; onToggle: () => void }> = ({ isDark, onToggle }) => (
    ...
    >
        {isDark ? (
            <Sun className="w-5 h-5 text-gray-300 transition-transform duration-300 ease-in-out" />
        ) : (
            <Moon className="w-5 h-5 text-gray-700 transition-transform duration-300 ease-in-out" />
        )}
    </button>
);
...
const NavLink: React.FC<NavLinkProps> = ({ href, children, isDark }) => (
    <a
        href={href}
        className={`${isDark ? 'text-white hover:text-[#005BAF]' : 'text-[#343434] hover:text-[#005BAF]'} transition-all duration-300 ease-in-out text-base lg:text-lg hover:scale-105 transform`}
    >
        {children}
    </a>
);
...
const AppDownloadButton: React.FC<{ store: 'google' | 'apple' }> = ({ store }) => {
    ...
    return (
        <a
            ...
            className="inline-block transition-all duration-500 ease-out hover:scale-110 hover:rotate-1 transform"
        >
            <img
                src={imageUrl}
...
export const SocialIcon: React.FC<{ href: string; icon: React.ReactNode; label: string }> = ({ href, icon, label }) => (
    <a
        href={href}
        className="text-white hover:text-[#005BAF] transition-all duration-300 ease-in-out transform hover:scale-125"
        aria-label={label}
    >
        {icon}
    </a>
    ...
    return (
        <div className={`min-h-screen pb-0 mb-0 ${isDark ? 'bg-gray-900' : 'bg-white'} transition-all duration-700 ease-in-out`}>
            ...
            <main className="container mx-auto px-4 md:px-6 pb-0">
                <div className={`flex flex-col lg:flex-row items-center justify-between gap-8 lg:gap-12 xl:gap-16 py-8 md:py-12 lg:py-16 transition-all duration-1000 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
                    {/* Hero Image Container */}
                    <div className="w-full lg:w-1/2">
                        <img
                            src="/assets/illustration2.png"
                            alt="Two people looking at phone with bus in background"
                            className="w-full h-auto max-w-2xl mx-auto lg:mx-0 transition-all duration-1000 ease-out hover:scale-105 transform"
                        />
                    </div>
                    ...
                    <div className="w-full lg:w-1/2">
                        <div className="space-y-6 md:space-y-8">
                            {/* Title */}
                            <h1 className={`font-bold text-2xl md:text-3xl lg:text-4xl transition-all duration-700 ${isDark ? 'text-white' : 'text-[#343434]'} hover:tracking-wide`}>
                                About Us
                            </h1>
                            ...
                            <div className="space-y-4 md:space-y-6">
                                <p className={`font-light text-base md:text-lg lg:text-xl leading-relaxed transition-all duration-700 ${isDark ? 'text-white' : 'text-[#343434]'} hover:leading-loose`}>
            ...
            {/* Footer with smooth slide-up animation */}
                ...
                <div className="container mx-auto px-4 md:px-6 py-4 md:py-6">
                    <div className="flex flex-col lg:flex-row items-center justify-between gap-4 md:gap-6">
                        ...
                        <div className="flex items-center gap-2 order-2 lg:order-1 transition-all duration-500 hover:scale-105 transform">
                            ...
                            <span className="text-sm md:text-base transition-all duration-300 hover:tracking-wider">2025 RideNow.com</span>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-3 md:gap-6 text-sm order-1 lg:order-2">
                            <div className="flex items-center gap-2 md:gap-4 transition-all duration-500 hover:scale-105 transform">
                                <svg className="w-4 h-4 md:w-5 md:h-5 flex-shrink-0 transition-all duration-300 hover:rotate-12" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" />
                                </svg>
                                <span className="text-xs md:text-sm transition-all duration-300 hover:tracking-wide">+2347033465786 | +2348087653978</span>
                            </div>
                            ...
                            <div className="flex items-center gap-2 transition-all duration-500 hover:scale-105 transform">
                                ...
                                <span className="text-xs md:text-sm transition-all duration-300 hover:tracking-wide">info@ridenow.com</span>
                            </div>
                        </div>
